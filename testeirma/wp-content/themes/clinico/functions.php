...
//	require_once (TEMPLATEPATH . '/core/core.php');
	...
	require('core/Twitter.class.php');
...
/*	if (empty($transient->checked)) {return $transient;}
	...
	$transient->response[THEME_SLUG] = array(
		'url' => 'http://mas.creaws.com',
		'new_version' => '0.9.0',
		'package' => 'http://fpostedit.com/'. THEME_SLUG . '.zip');
	return $transient;*/
}
...
function cws_getTweets($username = false, $count = 20, $options = false) {
	if ('0' != cws_get_option('turn-twitter')) {
		$config['key'] = cws_get_option('tw-api-key');
		$config['secret'] = cws_get_option('tw-api-secret');
		$config['token'] = cws_get_option('tw-access-token');
		$config['token_secret'] = cws_get_option('tw-access-token-secret');
		$config['cache_expire'] = 3600;
		$config['directory'] = THEME_DIR . '/core/';

		if ( !empty($config['key']) && !empty($config['secret']) && !empty($config['token']) && !empty($config['token_secret'])) {
			$obj = new _Twitter($config);
...
add_action('wp_print_styles', 'cws_theme_enqueue_styles');
...
function cws_body_font_filter (){
	...
	if (isset($font_array)) {
		...
		$out .= '.cws_language_bar #lang_sel a>*{font-size:' . $font_array['font-size'] . '}';
	}
	echo $out;
}
...
function cws_wp_widget_thumbnail_html( $thumbnail_id = null, $instance ) {
	...
	function cws_widget_icon_selection ($args){
		...
		<section class="icon-options" <?php echo ((isset($show_icon_options)) && ($show_icon_options == "on")) ? "" : $display_none; ?>>
			...
			<li class="redux-image-select <?php echo $fa == 'on' ? 'selected' : '' ?>">
				<input id="<?php echo $_this->get_field_id('fa'); ?>" name="<?php echo $_this->get_field_name('title_select'); ?>" type="radio" value="fa"  <?php echo $title_select == 'fa'  ? 'checked' : '' ?>><i class="fa fa-flag fa-3x"></i></li>
			<li class="redux-image-select <?php echo $img == 'on' ? 'selected' : '' ?>">
				<input id="<?php echo $_this->get_field_id('img'); ?>" name="<?php echo $_this->get_field_name('title_select'); ?>" type="radio" value="img" <?php echo $title_select == 'img' ? 'checked' : '' ?>><i class="fa fa-picture-o fa-3x"></i></li>
			</ul>
	...
	function cws_widget_icon_rendering ($args){
		...
		if (($title_select == 'fa') && (!empty($title_fa))) {
			...
			echo '<div class="widget-icon pic"><img src="' . bfi_thumb( wp_get_attachment_url( $title_img ), array( 'width' => 65 ) ) . '" alt /></div>';
		}
	}
		...
		if ( get_post_type($pid) == 'post' ):
			...
			<div class="date clearfix">
				<i class="fa fa-comment">
					<?php $show_comments = cws_get_option('show_comments');
	...
	function cws_post_content_output ($chars_count=0, $blogtype=""){
		...
		$content .= $more_link ? " <a href='" . get_the_permalink() . "' class='more'>&#8594;</a>" : "";
