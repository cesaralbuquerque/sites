...
function invokeSelect2() {
	...
	jQuery('.widget-content li.redux-image-select').click(function()
	{
		...
		var ind = _this.index();
		_this.parents(".widget-content").find(".image-part").children(".img-wrapper").eq(ind).show(300).siblings().hide(300);
	});
	...
	jQuery('a[id^="media"]').live("click", function() {
		...
		wp.media.editor.send.attachment = function(props, attachment) {
			...
			jQuery('img#img-'+_this).attr("src", url);
