...
function render_portfolio ($cols = 2,
		...
		$content = '';
		if (!empty($cats) && '*' !== $cats) {
			$cid = get_term_by('slug', $cats, 'cws-portfolio-type')->term_id;
			$categories = get_terms('cws-portfolio-type', array('hide_empty' => true, 'child_of' => $cid));
		} else {
		...
		if ( $UseFilter ) :
				...
				$ajaxurl = THEME_URI . '/core/portfolio-cols.php';
					...
					if (!empty($filter)) {
						$tax_query_arr = array(
								'taxonomy' => 'cws-portfolio-type',
								'field' => 'slug',
								'terms' => explode(',', $filter)
							);
						$bRemoveTaxQuery = false;
					} else {
						$bRemoveTaxQuery = true;
					}
					...
					if ($p->have_posts()) :
						...
						if ($bAjax) $content .= "<div class='ajax_content'>";
...
function render_portfolio_carousel ($postspp, $title){
	$out = '';
...
function build_portfolio_item ($pageid, $cols, $is_carousel = false, $thumbnail_dims, $chars_count ) {
	$out = '';
	$cws_stored_meta = get_post_meta( $pageid, 'cws-portfolio');
	if (!$is_carousel){
		$cats = wp_get_post_terms($pageid, 'cws-portfolio-type');
		$cat_id = '';
		if ( count($cats) > 0 ) {
			foreach ( $cats as $cat ) {
				$cat_id .= $cat->term_id;
