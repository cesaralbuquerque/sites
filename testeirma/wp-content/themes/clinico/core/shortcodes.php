...
$cws_shortcodes = array ('progress', 'embed', 'alert', 'cws_button', 'quote', 'fa', 'featured_fa', 'dropcap', 'mark', 'price-table', 'ourteam', 'portfolio', 'cws_cta', 'news', 'tweets', 'table', 'milestone', 'services', 'shortcode_carousel', 'shortcode_blog');
...
function price_table($attr) {
	...
	$args = array(
		...
		'tax_query' => array(
			...
			array(
				...
				'field' => 'slug',
				'terms' => array( 'post-format-table' )
			)
		)
	);
	if ( $cat ) array_push( $args['tax_query'],  array( 'taxonomy' => 'category',
														'field' => 'slug',
														'terms' => array( $cat ) ) );
	...
	if ( $query->have_posts() ) {
		global $post;
		$output = '<div class="pricing_table col-' . $columns . '">';
		$end = false;
		while ( $query->have_posts() ) :
				...
				$table_signup = isset( $cws_stored_meta[0]['cws-mb-table-signup'] ) ? esc_attr( $cws_stored_meta[0]['cws-mb-table-signup'] ) : '';
				$table_hilighted = isset( $cws_stored_meta[0]['cws-mb-table-hilighted'] ) ? ' active_table_column' : '';
				$title = get_the_title();

				$output .= '<div class="pricing_table_column' . $table_hilighted . '"><div>';
				$output .= '<div class="pricing_table_header">' . ( !empty( $title ) ? '<div class="title">'. esc_attr( $title ) .'</div>' : '' )
																. $table_encourage
																. '</div>';
				$output .= '<div class="price_part"><span class="price_container">' . ( !empty($table_currency) ? '<span class="currency">' . $table_currency . '</span>' : '' )
													  . ( !empty($table_price) ? '<span class="price">' . $table_price . '</span>' : '' )
													  . ( !empty($table_price_description) ? '<span class="price_description">' . $table_price_description . '</span>' : '' )
													  . '</span></div>';
				$output .= '<div class="content_part">';
				$text = get_the_content();
//				$text = str_replace("\r\n", "</p>\r\n<p>", $text);
				$output .= apply_filters( 'the_content',  $text);
				$output .= '</div>';
				$output .= '<a href="' . ( !empty($table_order) ? $table_order : '#' ) . '" class="cws_button large pricing_table_button">' . $table_signup . '</a>';
				$output .= '</div></div>';
...
function shortcode_twitter($attr) {
	...
	if ($twt_obj) {
		if (is_array($twt_obj)) {
			...
			$out = 'Twitter API keys and tokens are not set.';
		}
	}
	return $out;
}
...
function shortcode_services($attr) {
	...
	$include = !empty($filter) ? explode(',', $filter) : array();
...
function shortcode_cws_portfolio($attr = array()) {
	...
	if ($usecarousel == '1') {
		$content .= render_portfolio_carousel($postspp, $title);
	} else {
...
function cws_shortcode_blog ( $attr, $content ){
	...
	$cat_filter = function ($val) use ( $tech_cat_array ){
		return !in_array( $val, $tech_cat_array );
	};
	if ( count( $tech_cat ) > 0 ){
			array_walk ( $tech_cat, function ( &$val ){
			$val = '-' . $val;
		});
	...
	if ( !empty($cats) ){
		$cats = explode( ",", $cats );
		array_walk( $cats, function ( &$val ){
			$val = get_category_by_slug( $val )->term_id;
		});
		$cats = array_filter( $cats, $cat_filter );
		if ( count( $cats ) > 0 ){
			$cats = implode( $cats, "," );;
			if (isset($args["cat"])){
				$args["cat"] .= "," . $cats;
