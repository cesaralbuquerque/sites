...
/*						wp_register_style( 'redux-google-fonts', $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
		...
		*/
		...
		public function get_header_html( $field ) {
			...
			if ( $this->args['default_show'] === true && isset( $field['default'] ) && isset($this->options[$field['id']]) && $this->options[$field['id']] != $field['default'] && $field['type'] !== "info" && $field['type'] !== "section" && $field['type'] !== "editor" && $field['type'] !== "ace_editor" ) {
		...
		public function _validate_options( $plugin_options ) {
			...
			if( !empty( $plugin_options['import'] ) ) {
				...
				if ( !empty( $import ) ) {
					...
					if (isset($imported_options['cws-clinico'])) {
						$cws_options = $imported_options['cws-clinico'];
						global $wpdb;
						foreach ($cws_options as $opt) {
							$res = $wpdb->query( "INSERT INTO $wpdb->options (option_name, option_value) VALUES('{$opt['option_name']}', '{$opt['option_value']}') ON DUPLICATE KEY UPDATE option_value = '{$opt['option_value']}'" );
						}
						unset($imported_options['cws-clinico']);
					}
				}
			...
			*/
			...
			if( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] == 'true' && $saved == '1' ) {
				if( isset( $this->options['REDUX_imported'] ) && $this->options['REDUX_imported'] === 1 ) {
					...
					*/
					echo '<div id="redux-save"><strong>' . apply_filters( "redux-saved-text-{$this->args['opt_name']}", __( 'Settings Saved!<br>', 'redux-framework' ) ) . '</strong></div>';
				}
			}
			/**
