<?php
	if ( post_password_required() ) { ?>
		<p class="no-comments"><?php __('Sorry, this post is password protected.', 'clinico'); ?></p>
	<?php
		return;
	}
?>
...
?>
	<section class="comments-part">
			...
			<div class='widget-title'><?php echo __('Comments (', 'clinico') . get_comments_number() . ')'; ?></div>
...
<?php else :
	if ( !comments_open() ) : ?>
		<p class="no-comments"><?php __('Comments are closed.', 'clinico'); ?></p>
	<?php else: ?>
		<section class="comments-part">
				<?php comment_form($comment_form_args); ?>
		</section>
