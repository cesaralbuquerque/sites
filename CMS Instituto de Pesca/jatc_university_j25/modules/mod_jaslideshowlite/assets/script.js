...
var JASliderCSS = new Class({
	...
	initialize: function (element, options) {
		...
		jitems[0].clone().setStyles ({'position':'relative', 'visibility':'hidden', 'z-index': 1}).addClass('ja-ss-item-ghost').injectTop (jmain);
	...
	load: function(img, idx){
		img.store('loaded', 1);	//mark it as loaded
	...
	prepare: function(idx){
		...
		if(curImg.retrieve('loaded')){	//already loaded			
			...
			Asset.image(curImg.src, {
				onLoad: this.load.bind(this, [curImg, idx])
			});
		}
	},
